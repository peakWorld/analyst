  ...
  const Visitor: babel.Visitor = {
    ArrayExpression(path) {
      if (
        ...
        !t.isIdentifier(path.parent.id, { name: 'routes' })
      ) {
        return;
      }
      path.traverse({
        ObjectExpression(path) {
          ...
          if (properties && properties.length) {
            ...
            (properties as babel.types.ObjectProperty[]).forEach((item) => {
              if (
                t.isIdentifier(item.key, { name: 'path' }) &&
                t.isStringLiteral(item.value)
              ) {
                route = item.value.value;
              } else if (t.isIdentifier(item.key, { name: 'component' })) {
                if (
                  t.isArrowFunctionExpression(item.value) &&
                  t.isCallExpression(item.value.body) &&
                  t.isStringLiteral(item.value.body.arguments[0])
                ) {
